...
export default async function handler(req, res) {
  ...
  try {
    ...
    const dbRes = await fetch(`${FIREBASE_DB}/stollbergVerkaufAnfragen/${encodeURIComponent(anfrageId)}.json`);
    ...
    if (type === 'rejected') {
      ...
      recipients = REPLY_TO.length ? REPLY_TO : [];
      if (!recipients.length) return res.status(500).json({ error: 'MAIL_REPLY_TO nicht konfiguriert' });
    } else {
    ...
    await fetch(`${FIREBASE_DB}/stollbergVerkaufAnfragen/${encodeURIComponent(anfrageId)}.json`, {
      method: 'PATCH',
